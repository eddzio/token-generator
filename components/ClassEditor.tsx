    ...
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="px-4 py-3 border-b border-stone-200">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-xs text-stone-800 tabular-nums">
            {rows.length} {rows.length === 1 ? "rule" : "rules"}
          </span>
        </div>
        <button
          onClick={addRow}
          className="w-full px-3 py-2 text-xs font-medium tracking-wide uppercase bg-stone-100 text-stone-700 border border-stone-200 rounded hover:bg-stone-200 hover:border-stone-300 transition-colors flex items-center justify-center gap-2"
        >
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2">
        {rows.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-xs text-stone-800">No classes defined</p>
            <p className="text-xs text-stone-400 mt-1">Click &quot;Add Class&quot; to begin</p>
          </div>
        ) : (
          rows.map((row, index) => (
            <ClassRow
